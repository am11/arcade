...
def upsert_package(resolved, resolved_package, packages):
    """Insert or update the resolved list with the new package version, if necessary."""
    if resolved_package in resolved:
        existing_index = resolved.index(resolved_package)
        current_version = packages[resolved_package].get("Version", "0")
        existing_version = packages[resolved[existing_index]].get("Version", "0")

        if compare_debian_versions(current_version, existing_version) > 0:
            print(f"Overriding {resolved_package}. old version: {existing_version}, new version: {current_version}")
            resolved[existing_index] = resolved_package
    else:
...
def resolve_dependencies(packages, aliases, desired_packages):
    ...
    while to_process:
        ...
        upsert_package(resolved, resolved_package, packages)
...
def install_packages(mirror, packages_info, aliases, tmp_dir, extract_dir, ar_tool, desired_packages):
    ...
    packages_to_download = {pkg: packages_info[pkg] for pkg in resolved_packages if pkg in packages_info}
